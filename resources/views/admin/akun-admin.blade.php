...
<body style="margin-top: 64px;">
    ...
    <div class="pl-4 sm:ml-64">
        <div>
            ...
            @include('components.admin.ModalTambahAdmin')
        </div>
    </div>
    ...
    <script>
        ...
        @if (Session::has('tambahAdmin'))
            toastr.success("{{ Session::get('tambahAdmin') }}")
        @endif

        @if (Session::has('updateAdmin'))
            toastr.success("{{ Session::get('updateAdmin') }}")
        @endif

        @if (Session::has('hapusAdmin'))
            toastr.success("{{ Session::get('hapusAdmin') }}")
        @endif
    </script>
