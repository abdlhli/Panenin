                ...
                <tbody>
                    @foreach ($orders as $order)
                        <tr class="border-b hover:bg-gray-200">
                            ...
                            <td class="py-3">
                                <div class="flex px-4">
                                    <button id="TombolModalUpdateStatusProses"
                                        data-modal-toggle="ModalUpdateStatusProses{{ $order->id_pemesanan }}"
                                    ...
                                    <button id="TombolDetailPemesananProses" data-target="{{ $order->id_pemesanan }}"
                                        class="flex items-center hover:bg-gray-100 text-gray-500 hover:text-cyan-500 font-semibold py-1 px-4 rounded shadow">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                            viewBox="0 0 24 24" fill="none" stroke="#000000" stroke-width="1"
                                            stroke-linecap="round" stroke-linejoin="round">
                        ...
                        <tr class="border-b hover:bg-gray-200">
                            <!-- Kolom lainnya -->
                            <td colspan="7" id="DetailPemesananProses{{ $order->id_pemesanan }}" class="hidden">
                        ...
                        @include('components.pemesanan.ModalUpdatePemesananProses')
                    @endforeach
                </tbody>
            </table>
        </div>
...
<script>
    ...
    // Ambil semua tombol "TombolDetailPemesananProses"
    const tombolDetailPemesananProses = document.querySelectorAll('#TombolDetailPemesananProses');

    // Loop melalui setiap tombol
    tombolDetailPemesananProses.forEach((tombol) => {
        // Tambahkan event listener untuk klik
        tombol.addEventListener('click', (e) => {
            e.preventDefault();

            // Ambil target ID dari atribut "data-target"
            const targetIDProses = tombol.getAttribute('data-target');

            // Ambil elemen detail pemesanan dengan ID yang sesuai
            const detailPemesananProses = document.getElementById(
                `DetailPemesananProses${targetIDProses}`);

            // Toggle kelas "hidden" pada elemen detail pemesanan untuk mengubah tampilan
            detailPemesananProses.classList.toggle('hidden');
        });
    });
</script>
