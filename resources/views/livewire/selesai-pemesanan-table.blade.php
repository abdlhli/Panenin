                ...
                <tbody>
                    @foreach ($orders as $order)
                        <tr class="border-b hover:bg-gray-200">
                            ...
                            <td class="py-3">
                                <div class="flex px-4">
                                    <button id="TombolModalUpdateStatusSelesai"
                                        data-modal-toggle="ModalUpdateStatusSelesai{{ $order->id_pemesanan }}"
                                    ...
                                    <button id="TombolDetailPemesananSelesai" data-target="{{ $order->id_pemesanan }}"
                                        class="flex items-center hover:bg-gray-100 text-gray-500 hover:text-cyan-500 font-semibold py-1 px-4 rounded shadow">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                            viewBox="0 0 24 24" fill="none" stroke="#000000" stroke-width="1"
                                            stroke-linecap="round" stroke-linejoin="round">
                        ...
                        <tr class="border-b hover:bg-gray-200">
                            <!-- Kolom lainnya -->
                            <td colspan="7" id="DetailPemesananSelesai{{ $order->id_pemesanan }}" class="hidden">
                        ...
                        @include('components.pemesanan.ModalUpdatePemesananSelesai')
                    @endforeach
                </tbody>
            </table>
        </div>
...
<script>
    ...
    // Ambil semua tombol "TombolDetailPemesananSelesai"
    const tombolDetailPemesananSelesai = document.querySelectorAll('#TombolDetailPemesananSelesai');

    // Loop melalui setiap tombol
    tombolDetailPemesananSelesai.forEach((tombol) => {
        // Tambahkan event listener untuk klik
        tombol.addEventListener('click', (e) => {
            e.preventDefault();

            // Ambil target ID dari atribut "data-target"
            const targetIDSelesai = tombol.getAttribute('data-target');

            // Ambil elemen detail pemesanan dengan ID yang sesuai
            const detailPemesananSelesai = document.getElementById(
                `DetailPemesananSelesai${targetIDSelesai}`);

            // Toggle kelas "hidden" pada elemen detail pemesanan untuk mengubah tampilan
            detailPemesananSelesai.classList.toggle('hidden');
        });
    });
</script>
